...
from .image_service import save_image_metadata  # For saving metadata after download
...
async def edit_image_from_prompt(
    ...
    Args:
        ...
        image_bytes_list: List of source image file contents as bytes (must be PNG/WEBP/JPG, <= 25MB each).
    ...
    """
    logger.info(f"Requesting image edit for prompt: '{prompt}' with size={size}, quality={quality}, n={n}, images={len(image_bytes_list)}")
    try:
        # Prepare image parameters - gpt-image-1 supports multiple images
        image_params = [(f"image_{i}.png", img_data, "image/png") for i, img_data in enumerate(image_bytes_list)]
        ...
        # Extract base64 data from response
        image_data_list = [item.b64_json for item in response.data]
        logger.info(f"Image(s) edited successfully. Number of images: {len(image_data_list)}")
...
async def save_image_from_base64(
    ...
    try:
        ...
        for idx, image_data in enumerate(image_data_list):
            ...
            save_image_metadata(metadata) # This is currently synchronous, might need async version
